...
impl Event {
    /// Return the string representing this event's code.
    pub fn code_as_ref(&self) -> &str {
        match self {
            Event::Synchronization(code) => code.as_ref(),
            Event::Key(code) => code.as_ref(),
            Event::RelativeAxis(code) => code.as_ref(),
            Event::AbsoluteAxis(code) => code.as_ref(),
            Event::Miscellaneous(code) => code.as_ref(),
            Event::Switch(code) => code.as_ref(),
            Event::LED(code) => code.as_ref(),
            Event::Sound(code) => code.as_ref(),
            Event::AutoRepeat(code) => code.as_ref(),
            Event::ForceFeedback(code) => code.as_ref(),
            Event::ForceFeedbackStatus(code) => code.as_ref(),
        }
    }
}
